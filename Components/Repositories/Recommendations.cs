...
public class Recommendations 
{
    ...
    private const int MAX_RECOMMENDATIONS = 25;
    ...
    public async Task<List<Book>> ResearchBookRecommendations(string UserName){
        var t = await GetBookRecommendations(UserName);
        List<Book> scoredBooks = new();
        foreach(var book in scoredBooks){
            if(book != null){
                if(book.author_name != null){
                    foreach(var author in book.author_name){
                        scoredBooks.AddRange(await SearchBookByApi(author));
                        await Task.Delay(500);
                    }
                }
            }
        }
        scoredBooks = await GetBookRecommendations(UserName);
        return scoredBooks;
    }
    public async Task<List<Book>> SearchBookByApi(string query){
        try{
            ...
            var response = await client.GetAsync($"https://openlibrary.org/search.json?author={query}");
